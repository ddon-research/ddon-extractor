...
/**
 * Blowfish encryption (ECB, CBC and CTR mode) as defined by Bruce Schneier here: <a href="http://www.schneier.com/paper-blowfish-fse.html">http://www.schneier.com/paper-blowfish-fse.html</a>
 * <p>
 * Complies with test vectors found here: <a href="http://www.schneier.com/code/vectors.txt">http://www.schneier.com/code/vectors.txt</a>
 * <p>
 * non-standard mode provided to be usable with the javascript crypto library found here: <a href="http://etherhack.co.uk/symmetric/blowfish/blowfish.html">http://etherhack.co.uk/symmetric/blowfish/blowfish.html</a>
 * <p>
 * By Taylor Hornby, 1/7/1010
 * Code is partly adopted from the javascript crypto library by Daniel Rench
 * compat mode provided complies with the definition of the mcrypt definition found here: <a href="https://github.com/ftofficer/python-mcrypt-idea/blob/master/libmcrypt-2.5.8/modules/algorithms/blowfish-compat.c">https://github.com/ftofficer/python-mcrypt-idea/blob/master/libmcrypt-2.5.8/modules/algorithms/blowfish-compat.c</a>
 * By Mikeprod 2014/08/25
 * <p>
 * USAGE:
 * <p>
 * Provide the key when creating the object. The key can be any size up to 448 bits.
 * The key can be given as an array of bytes.
 * <code>BlowFish b = new BlowFish("04B915BA43FEB5B6");</code>
 * <p>
 * The plainText can be either a String or byte array.
 * <p>
 * <code>String plainText = "The quick brown fox jumped over the lazy dog.";</code>
 * <p>
 ...
 * Encrypting the same data with the same key will result in the same ciphertext. This mode should NOT be used unless necessary.
 * <p>
 * ---CBC---
 ...
 * Like ECB mode, with the added protection of an IV to make sure the same plaintext encrypted with the same key yields a different result.
 * The counter ensures that no patterns will be visible. CTR mode is secure and can be optimized for multithreaded applications.
 * <p>
 * For more information on cipher modes of operation, see <a href="http://en.wikipedia.org/wiki/Block_cipher_modes_of_operation">http://en.wikipedia.org/wiki/Block_cipher_modes_of_operation</a>
 * <p>
 * Things to remember
 * -Always use unique initialization vectors when using CBC and CTR mode. SetRandomIV will do the job for you.
 * -Blowfish is only as secure as the key you provide. When deriving a key from a password, use a secure hash function such as SHA-256 to create the key.
 ...
 * -Do not use the compatibility mode unless necessary.
 */
public class BlowFishCompat {
    /**
     * Standard is 16, to increase the number of rounds, bf_P needs to be equal to the number of rounds.
     * Use digits of PI.
     */
     ...
     */
    public BlowFishCompat(byte[] cipherKey) {
        setupKey(cipherKey);
    }
    ...
    public byte[] encryptECB(byte[] pt) {
        return cryptECB(pt, false);
    }

    /**
     * Decrypts a byte array (ECB)
     *
     * @param ct Ciphertext byte array
     * @return Plaintext
     */
    public byte[] decryptECB(byte[] ct) {
        return cryptECB(ct, true);
    }

    /**
     * Sets up the S-blocks and the key
     *
     * @param cipherKey Block cipher key (1-448 bits)
     */
    private void setupKey(byte[] cipherKey) {
        ...
        if (cipherKey.length > 56) {
            throw new RuntimeException("Key too long. 56 bytes required.");
        }
        System.arraycopy(cipherKey, 0, key, 0, cipherKey.length);
        int j = 0;
        for (int i = 0; i < 18; i++) {
            long d = ((key[j % cipherKey.length] * 256L + key[(j + 1) % cipherKey.length]) * 256L + key[(j + 2) % cipherKey.length]) * 256L + key[(j + 3) % cipherKey.length];
            bfP[i] = bfP[i] ^ d;
     ...
     *
     * @param text    plain/ciphertext
     * @param decrypt true to decrypt, false to encrypt
     * @return En-/Decrypted data
     */
    private byte[] cryptECB(byte[] text, boolean decrypt) {
     ...
     *
     * @param block The 64 bit block to encrypt
     */
     ...
     *
     * @param block The 64 bit block to decrypt
     */
    ...
    /**
     * Splits the block into the two long values
     *
     * @param block the 64 bit block to set up
     */
    ...
    /**
     * Converts the two long values into a 64 bit block
     *
     * @param block 64 bit buffer to receive the block
     */
    private void getBlock(byte[] block) {
        byte[] block1 = Arrays.reverse(toBytes(xlPar));
        byte[] block2 = Arrays.reverse(toBytes(xrPar));
     ...
     */
    private long round(long a, long b, int n) {
        long x1 = (bfS0[wordByte0(b)] + (bfS1[wordByte1(b)])) ^ bfS2[wordByte2(b)];
        long x2 = x1 + bfS3[wordByte3(b)];
        long x3 = x2 ^ bfP[n];
        return x3 ^ a;
    }

    /**
     * gets the first byte in a long
     */
    private int wordByte0(long w) {
        return (int) (w >> 24) & 0xff;
    }

    /**
     * gets the second byte in a long
     */
    private int wordByte1(long w) {
        return (int) (w >> 16) & 0xff;
    }

    /**
     * gets the third byte in a long
     */
    private int wordByte2(long w) {
        return (int) (w >> 8) & 0xff;
    }

    /**
     * gets the fourth byte in a long
     */
    private int wordByte3(long w) {
        return (int) w & 0xff;
    }
}
