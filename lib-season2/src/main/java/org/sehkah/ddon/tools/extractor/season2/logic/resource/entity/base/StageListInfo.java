...
public class StageListInfo {
    ...
    public StageListInfo(long stageNo, long type, int recommendLevel, long messageId, long version) {
        this(
                stageNo, Long.parseLong(ResourceMetadataLookupUtilSeason2.getMessageKey(ResourceFileLookupType.STAGE_LIST, messageId).replace("STAGE_NAME_", "")),
                type, StageInfoType.of(type),
                recommendLevel,
                messageId, ResourceMetadataLookupUtilSeason2.getMessage(ResourceFileLookupType.STAGE_LIST, messageId),
                version);
    }
}
