...
import org.sehkah.ddon.tools.extractor.season3.logic.resource.entity.quest.meta.ParamCallAnnounceType;
...
public class AIFSMDeserializer extends ClientResourceFileDeserializer<AIFSM> {
    ...
    private static FSMBaseParamCallMessage readFSMBaseParamCallMessage(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        long MsgType = XfsDeserializer.readUnsignedInteger(bufferReader);
        ParamCallMessageType MsgTypeName = ParamCallMessageType.of((int) MsgType);
        long QuestId = XfsDeserializer.readUnsignedInteger(bufferReader);
    ...
    private static FSMBaseParamCallMessageFortDefCommon readFSMBaseParamCallMessageFortDefCommon(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        FSMBaseParamCallMessage callMessage = readFSMBaseParamCallMessage(bufferReader, lookupUtil);
        long Type = XfsDeserializer.readUnsignedInteger(bufferReader);
        int GotoPointNo = XfsDeserializer.readSignedInteger(bufferReader);
    ...
    private static FSMBaseParamCallMessageFortDefBattleStart readFSMBaseParamCallMessageFortDefBattleStart(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        FSMBaseParamCallMessage callMessage = readFSMBaseParamCallMessage(bufferReader, lookupUtil);
        long Type = XfsDeserializer.readUnsignedInteger(bufferReader);
        int GotoPointNo = XfsDeserializer.readSignedInteger(bufferReader);
    ...
    private static FSMBaseParamCallNpcMessage readFSMBaseParamCallNpcMessage(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        long Type = XfsDeserializer.readUnsignedInteger(bufferReader);
        long QuestId = XfsDeserializer.readUnsignedInteger(bufferReader);
        long MsgNo = XfsDeserializer.readUnsignedInteger(bufferReader);
        ...
        return new FSMBaseParamCallNpcMessage(Type, QuestId, QuestName, MsgNo);
    }

    private static FSMOrderParamCallAnnounce readFSMOrderParamCallAnnounce(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        long Type = XfsDeserializer.readUnsignedInteger(bufferReader);
        ParamCallAnnounceType TypeName = ParamCallAnnounceType.of((int) Type);
        long QuestId = XfsDeserializer.readUnsignedInteger(bufferReader);
        long MsgNo = XfsDeserializer.readUnsignedInteger(bufferReader);
        ...
        return new FSMOrderParamCallAnnounce(Type, TypeName, QuestId, QuestName, MsgNo);
    }
    ...
    private static FSMOrderParamCallEvent readFSMOrderParamCallEvent(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        ...
        return new FSMOrderParamCallEvent(StageNo, StageName, EventNo, NpcArray);
    }
    ...
    private static FSMOrderParamCamera readFSMOrderParamCamera(BufferReader bufferReader) {
        ...
        return new FSMOrderParamCamera(ActType, CamRes, IsBtlNoCancel, CamId1, CamId2, CameraEnemyActType, CantActPl, CamId3);
    }
...
//        int MotionSeOffBank = XfsDeserializer.readSignedInteger(bufferReader);
    ...
    private static FSMOrderParamSetLayout readFSMOrderParamSetLayout(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        ...
        return new FSMOrderParamSetLayout(ActType, StageNo, StageName, FlagNo);
    }
    ...
    private static FSMOrderParamSetEnemySetEmInfo readFSMOrderParamSetEnemySetEmInfo(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        ...
        // Season 3 exclusive
        long ArmedSetMode = XfsDeserializer.readUnsignedInteger(bufferReader);
        boolean IsNoSetPS3 = XfsDeserializer.readBoolean(bufferReader);
        //
    ...
    private static FSMUnitParamSetDrawingSword readFSMUnitParamSetDrawingSword(BufferReader bufferReader) {
        boolean IsDrawingSword = XfsDeserializer.readBoolean(bufferReader);

        return new FSMUnitParamSetDrawingSword(IsDrawingSword);
    }

    private static FSMUnitParamSetStageEffect readFSMUnitParamSetStageEffect(BufferReader bufferReader) {
        long IndexNo = XfsDeserializer.readUnsignedInteger(bufferReader);
        long ElementNo = XfsDeserializer.readUnsignedInteger(bufferReader);

        return new FSMUnitParamSetStageEffect(IndexNo, ElementNo);
    }

    private static FSMUnitParamAddMenuEvidence readFSMUnitParamAddMenuEvidence(BufferReader bufferReader) {
        long IconId = XfsDeserializer.readUnsignedInteger(bufferReader);
        long NameMsgId = XfsDeserializer.readUnsignedInteger(bufferReader);
        long DetailMsgId = XfsDeserializer.readUnsignedInteger(bufferReader);

        return new FSMUnitParamAddMenuEvidence(IconId, NameMsgId, DetailMsgId);
    }
    ...
    private static AICopiableParameter readAICopiableParameter(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil, XfsHeader xfsHeader, String containerName) {
        ...
        return switch (containerName) {
            ...
            // Season 3 event exclusive
            case "SetDrawingSword" -> readFSMUnitParamSetDrawingSword(bufferReader);
            case "SetStageEffect" -> readFSMUnitParamSetStageEffect(bufferReader);
            case "AddMenuEvidence" -> readFSMUnitParamAddMenuEvidence(bufferReader);
            case "CallMenuEvidence", "CloseMenuEvidence", "ResetCountState", "FinishStageEffect" -> null;
            ...
            case "CallMessage" -> readFSMBaseParamCallMessage(bufferReader, lookupUtil);
            case "CallMessageFortDef_Common" -> readFSMBaseParamCallMessageFortDefCommon(bufferReader, lookupUtil);
            case "CallMessageFortDef_BattleStart" ->
                    readFSMBaseParamCallMessageFortDefBattleStart(bufferReader, lookupUtil);
            ...
            case "CallNpcMessage" -> readFSMBaseParamCallNpcMessage(bufferReader, lookupUtil);
    ...
    private static AIConditionTreeConstS32Node readAIConditionTreeConstS32Node(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil, XfsHeader xfsHeader) {
        long ChildListNum = bufferReader.readUnsignedInteger();
        List<AIConditionTreeNode> ChildList = bufferReader.readFixedLengthArray(ChildListNum, br -> readAIConditionTreeNode(br, lookupUtil, xfsHeader));

        int Value = XfsDeserializer.readSignedInteger(bufferReader);
        boolean IsBitNo = XfsDeserializer.readBoolean(bufferReader);

        return new AIConditionTreeConstS32Node(ChildList, Value, IsBitNo);
    }
    ...
    private static AIConditionTreeNode readAIConditionTreeNode(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil, XfsHeader xfsHeader) {
        ...
        return switch (xfsHeader.getClassHeaderIndex().get(xfsObjectData.getClassIndex()).getResourceName()) {
            ...
            case "rAIConditionTree::ConstS32Node" ->
                    readAIConditionTreeConstS32Node(bufferReader, lookupUtil, xfsHeader);
            default ->
