...
public class EventParamDeserializer extends ClientResourceFileDeserializer<EventParamList> {
    ...
    private static EventParam readEventParam(BufferReader bufferReader, ResourceMetadataLookupUtil lookupUtil) {
        ...
        Translation EventName = null;
        if (lookupUtil != null) {
            StageName = lookupUtil.getStageNameByStageNo(Stage);
            QuestName = lookupUtil.getQuestName(QuestId);
            EventName = lookupUtil.getMessageTranslation(GUIMessageLookupTable.EVENT_NAME.getFilePath(), "EVENT_NAME_ " + FileName.substring(FileName.lastIndexOf("\\") + 1));
        }

        return new EventParam(Type, TypeName, Stage, StageName, EvNo, EventName, Flag, FlagTypes, FileName, QuestId, QuestName, LightCtrl, LightCtrlName, StartFadeType, EndFadeType, SubMixerBefore, SubMixerAfter, OmAQCScale, OmList);
    }
